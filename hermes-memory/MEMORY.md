...
§
OA报销：查完必须把待报销金额写结果文件+记忆摘要（教训：只记流程不记结果）；流程细节见 skill smardaten-oa。
§
...
§
帛飞特项目（双验收）：甲方=杭州集控总集（PM吴恒、老板蔡总），业主=帛飞特；数睿实际全责执行（PM任可，7月调研另有刘乐；售前朱琳、商务俞亚君）；原报价10万（假设集控PM主导）→售前重估80万（执行全数睿，PM投入已超30人天/报价仅10人天）；核心矛盾=合同责任链（数睿→集控→业主）与实际执行错位；政府验收=主流程跑通（8.13前获蔡总认可）、合同验收=业主真正用起来（80万核心；近两周业主培训已完成，当前阻塞点=达到业主使用）；集控PM应然定位=共同调研→引导业主需求深浅→拍板需求程度（现状是丢给数睿直接对接业主，属责权倒挂需纠正）；集控PM决策责任=需求深度/政府验收时间与业主对清/业主商务接口；边界文档=98 hermes/帛飞特项目_数睿与集控PM工作边界_20260828.md（基本盘已改文字版，含落地动作章：朱琳/俞亚君对接集控）